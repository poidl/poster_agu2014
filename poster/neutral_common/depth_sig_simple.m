function [sns,ctns,pns] = depth_sig_simple(s0,ct0,p0,s,ct,p,drho)

% s0... bottle salinity (scalar or array of size (1,nx))
% sc... cast salinity (vector of size (nz,1) or array of size (nz,nx))
% 
% ss, cts, ps ... salinity, temperature and pressure on the cast(s) with identical potential density as the bottle. 
%     
% Note that the respective potential density is the one referenced to the bottle pressure. 
% It usually does not make sense to supply a bottle vector in which elements differ.
...
while any(~done)
    cnt=cnt+1;
    %pmid=0.5*(p0+p_); 
    bottle = gsw_rho(s0,ct0,p0);
    cast=gsw_rho(s_,ct_,p0);
    F=cast-bottle+drho;
