function gamma_i = gamma_3d_v1(s,ct,p,lon,lat)
...
if ~reuse_intersections
    make_intersections(s,ct,p); % neutral segments produced by gamma_i variations. expensive.
%    make_intersections_new(s,ct,p); 
%    keyboard
else
    if ~exist('./data/intersections.mat','file') 
        make_intersections(s,ct,p); % neutral segments produced by gamma_i variations. very expensive.
    end
end
